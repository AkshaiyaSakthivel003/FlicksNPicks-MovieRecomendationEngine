...
const MovieDetailInfo = ({ movie }) => {
  const formattedVoteAverage = movie ? `${movie.vote_average.toFixed(2)} ` : '';
...
const MovieDetails = () => {
  ...
  if (isCurrentlyFavorite) {
    updatedFavorites = favorites.filter((fav) => fav.id !== currentMovieDetail?.id);
  } else {
    updatedFavorites = [...favorites, currentMovieDetail];
  }
